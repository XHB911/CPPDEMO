...
const char* doc_root = "/home/hb/www/html";
...
bool http_conn::write() {
	...
	while (1) {
		...
		if (temp <= -1) {
			// 如果 TCP 写缓冲没有空间，则等待下一轮 EPOLLOUT 事件，在此期间，服务器器无法立即接收到同一客户的下一个请求，但这可以保证连接的完整性
			if (errno == EAGAIN) {
				modfd(m_epollfd, m_sockfd, EPOLLOUT);
				return true;
			}
...
http_conn::HTTP_CODE http_conn::parse_headers(char* text) {
	// 遇到空行，表示头部字段解析完毕
	if (text[0] == '\0') {
		// 如果HTTP 请求有消息体，则还需要读取 m_connten_length 字节的消息体，状态转移到 CHECK_STATE_CONTENT 状态
...
// 不解析真正的 HTTP 请求的消息体，只是判断是否被完整的读入了
...
// 当得到一个完整的、正确的 HTTP 请求时，就分析目标文件的属性，如果目标文件存在，且对所有用户可读，不是目录，则使用mmap将其映射到内存地址 m_file_address 上，并告诉调用者获取文件成功
...
bool http_conn::add_headers(int content_length) {
	if (add_content_length(content_length) && add_linger() && add_blank_line()) return true;
	return false;
}
